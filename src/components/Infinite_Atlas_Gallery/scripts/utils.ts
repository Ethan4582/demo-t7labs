...
export const createTextTexture = (project: Project): THREE.CanvasTexture => {
   ...
   if (project.Tags && project.Tags.length > 0) {
      const tagFontSize = 22; ctx.font = `500 ${tagFontSize}px ${monoFont}`;
      const pillH = 36; const pillPadX = 14; const pillGap = 10;
      ...
      project.Tags.forEach((tag, idx) => {
         ...
         if (!isFirst) {
            ctx.beginPath();
            const r = pillH / 2;
            ctx.moveTo(curX + r, bottomY); ctx.lineTo(curX + pillW - r, bottomY);
            ...
            ctx.strokeStyle = tagBorder; ctx.lineWidth = 1.5; ctx.stroke();
         }
   ...
   Object.assign(texture, {
      ...
      flipY: false, generateMipmaps: false, format: THREE.RGBAFormat,
   });
   return texture;
};
...
export const createTextureAtlas = (textures: THREE.Texture[], isText = false): THREE.CanvasTexture => {
   ...
   for (let slot = 0; slot < totalSlots; slot++) {
      ...
      try {
         if (isText) ctx.drawImage(texture.image as HTMLCanvasElement, x, y, textureSize, textureSize);
         else ctx.drawImage(texture.image as HTMLImageElement, x, y, textureSize, textureSize);
      } catch {
         if (!isText) { ctx.fillStyle = "#1c1c2e"; ctx.fillRect(x, y, textureSize, textureSize); }
      }
   }
      ...
      projects.forEach((project: Project, projectIndex: number) => {
         ...
         const texture = textureLoader.load(
            ...
            () => {
               ...
               if (ctx) { ctx.fillStyle = project.bgColor || "#1a1a2e"; ctx.fillRect(0, 0, 64, 64); }
